...
public class SecuritySql {
    ...
    public static void permissionSql(List<Permission> permissionList, File outFile, String dbName, String client, String userId) {
        ...
        long time = DateTimeUtils.currentTimeMillis();
        String sql = "INSERT INTO `{}`.`mx_security_permission` (`id`,`resource_id`, `client`, `group_name`, `permission_name`, `permission_code`, `create_user`, `create_time`, `last_modify_user`, `last_modify_time`) VALUES ('{}', '{}', '{}', '{}', '{}', '{}', {}, '{}', {});";
        permissionList.forEach(permission -> sqlList.add(StrUtil.format(sql, dbName, UUIDUtils.UUID(), client, client, permission.getGroupName(), permission.getName(), permission.getCode(), userId, time, userId, time)));
        FileWriter.create(outFile, Charset.defaultCharset()).writeLines(sqlList);
    }
