...
document.write('<link rel="stylesheet" type="text/css" href="/statics/plugins/bootstrap-multitabs/multitabs.min.css?v=' + version + '">');
...
document.write('<script type="text/javascript" src="/statics/js/system/index.min.js?v=' + version + '"><\/script>');
...
document.write('<script type="text/javascript" src="/statics/plugins/bootstrap-multitabs/multitabs.min.js?v=' + version + '"><\/script>');
document.write('<script type="text/javascript" src="/statics/js/custom/store.js?v=' + version + '"><\/script>');
document.write('<script type="text/javascript" src="/statics/js/custom/tools.js?v=' + version + '"><\/script>');
...
document.write('<script type="text/javascript" src="/statics/js/apis/admin.js?v=' + version + '"><\/script>');
document.write('<script type="text/javascript" src="/statics/js/custom/lock.js?v=' + version + '"><\/script>');

function openParentModal(dom) {
    // 获取父页面的 modal 元素
    const modal = parent.document.querySelector(dom);
    if (modal) {
        // 触发 Bootstrap 5 的 Modal
        const modalInstance = new parent.bootstrap.Modal(modal);
        modalInstance.show();
    } else {
        console.error('未找到父页面的 '+dom+' 元素');
    }
}

function openModal(dom) {
    // 获取父页面的 modal 元素
    const modal = document.querySelector(dom);
    if (modal) {
        // 触发 Bootstrap 5 的 Modal
        const modalInstance = new bootstrap.Modal(modal);
        modalInstance.show();
    } else {
        console.error('未找到页面的 '+dom+' 元素');
    }
}