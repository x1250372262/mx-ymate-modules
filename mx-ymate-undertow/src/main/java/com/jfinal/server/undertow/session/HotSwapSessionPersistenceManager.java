...
public class HotSwapSessionPersistenceManager implements SessionPersistenceManager {
                    ...
                    if (sessionEntry.getValue().expiry.getTime() > time) {
                        ...
                        for (Map.Entry<String, byte[]> sessionAttribute : sessionEntry.getValue().data.entrySet()) {
                            // final ObjectInputStream in = new ObjectInputStream(new ByteArrayInputStream(sessionAttribute.getValue()));
                        	    ClassLoader cl = undertowConfig.getClassLoader();
                        	    final HotSwapObjectInputStream in = new HotSwapObjectInputStream(new ByteArrayInputStream(sessionAttribute.getValue()), cl);
